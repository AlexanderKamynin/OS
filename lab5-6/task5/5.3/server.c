...
int main(int argc, char** argv) {
        ...
        struct sembuf waitNotEmpty = {1, -1, 0};
        ...
        struct sembuf waitNotFull = {0, -1, 0};
        ...
        // 1 - число свободных ячеек - считающий. Захватывается процессами-писателями перед работой - ждут освобождения первой свободной ячейки
        // 2 - число занятых ячеек - считающий. Захыватывается процессами-читателями перед работой - ждут порцию данных
        // 3 - работа с памятью. Нужен для атомарных операций в предыдущих двух семафорах, т.к. они не бинарные
        ...
        int res;
        if((res = semctl(semaphore, 0, SETVAL, BUF_SIZE)))
        {
                printf("Error while set arg in first semaphore\n");
                kill(getpid(),SIGINT);
        }

        if((res = semctl(semaphore, 1, SETVAL, 0)))
        {
                printf("Error while set arg in second semaphore\n");
                kill(getpid(),SIGINT);
        }

        if((res = semctl(semaphore, 2, SETVAL, 1)))
        {
                printf("Error while set arg in third semaphore\n");
                kill(getpid(),SIGINT);
        }
